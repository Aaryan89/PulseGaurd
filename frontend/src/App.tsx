  ...
  <nav className="bg-slate-900 text-white p-4">
    <div className="max-w-6xl mx-auto flex justify-between items-center">
      ...
      <div className="flex space-x-6">
        <Link to="/" className="hover:text-blue-300">Merchants</Link>
        <Link to="/cost" className="hover:text-blue-300">Cost Curve</Link>
      </div>
    </div>
  </nav>
);
...
const MerchantList = () => {
  ...
  useEffect(() => {
    axios.get('http://localhost:8000/merchants').then(res => setMerchants(res.data));
  }, []);
  ...
  return (
    <div className="p-8 max-w-6xl mx-auto">
      <div className="flex justify-between items-center mb-6">
        ...
        <button 
          onClick={() => {
            axios.post('http://localhost:8000/refresh').then(() => alert('Pipeline refreshing in background. Reload page in a few seconds.'));
          }}
      ...
      <div className="bg-white rounded-lg shadow border border-gray-200 overflow-hidden">
        <table className="w-full text-left border-collapse">
          ...
          <tbody>
            {merchants.map(m => (
              <tr key={m.merchant_id} className="border-b border-gray-100 hover:bg-gray-50">
                <td className="p-4 text-blue-600 font-medium">
                  <Link to={`/merchant/${m.merchant_id}`}>{m.merchant_id}</Link>
                </td>
                <td className="p-4">
                  {m.status === 'normal' && <span className="inline-flex items-center px-2.5 py-0.5 rounded-full text-xs font-medium bg-green-100 text-green-800">Normal</span>}
                  {m.status === 'watch' && <span className="inline-flex items-center px-2.5 py-0.5 rounded-full text-xs font-medium bg-yellow-100 text-yellow-800">Watch</span>}
                  {m.status === 'flagged' && <span className="inline-flex items-center px-2.5 py-0.5 rounded-full text-xs font-medium bg-red-100 text-red-800">Flagged</span>}
                </td>
...
const MerchantDetail = () => {
  ...
  useEffect(() => {
    axios.get(`http://localhost:8000/merchants/${id}/timeline`).then(res => setTimeline(res.data));
    axios.get(`http://localhost:8000/merchants/${id}/flags`).then(res => setFlags(res.data));
  }, [id]);
  ...
  return (
    <div className="p-8 max-w-6xl mx-auto space-y-8">
      <div>
        <Link to="/" className="text-blue-600 hover:underline mb-2 inline-block">&larr; Back to Overview</Link>
        <h1 className="text-3xl font-bold">{id} Risk Detail</h1>
      </div>

      <div className="bg-white p-6 rounded-lg shadow border border-gray-200">
        <h2 className="text-lg font-bold mb-4">Volume & Ticket Size Timeline</h2>
        <div className="h-72">
            ...
            <LineChart data={timeline.timeline}>
              ...
              {timeline.flagged_windows.map((fw: any, idx: number) => (
                <ReferenceArea 
                  ...
                  yAxisId="left" 
                  x1={fw.timestamp} 
                  x2={new Date(new Date(fw.timestamp).getTime() + 3600000).toISOString()} // 1 hour wide
                  fill="#ef4444" 
                  fillOpacity={0.2} 
                />
              ))}
      ...
      <div className="bg-white p-6 rounded-lg shadow border border-gray-200">
        <h2 className="text-lg font-bold mb-4">Flagged Anomalies (Stage 2 Isolation Forest)</h2>
        <div className="mb-6 bg-red-50 p-4 rounded text-red-900 border border-red-100">
          ...
          <ul className="list-disc pl-5 space-y-1 text-sm">
            {flags.audit_log.slice(0, 10).map((log: str, i: number) => (
              <li key={i}>{log}</li>
            ))}
            {flags.audit_log.length > 10 && <li>...and {flags.audit_log.length - 10} more regime breaks</li>}
          </ul>
        </div>
          ...
          <tbody>
            {flags.flagged_transactions.slice(0, 50).map((txn: any) => (
              <tr key={txn.transaction_id} className="border-b border-gray-100">
                ...
                <td className="p-3 font-medium text-red-600">{txn.score.toFixed(3)}</td>
              </tr>
            ))}
          </tbody>
        </table>
        {flags.flagged_transactions.length > 50 && (
          <div className="text-center p-4 text-gray-500 text-sm">Showing 50 of {flags.flagged_transactions.length} transactions</div>
        )}
      </div>
    </div>
  );
